...
def naive_open_loop(bimanual_kuka: BimanualKuka, qgoal_thanos, qgoal_medusa, angle = 30):
    ...
    bimanual_kuka.se2_arms_open_loop(qgoal_thanos, np.array([0,0,np.pi]), medusa=False, se2_time = 20.0, force = 0.0, object_kg = 2.0, filter_vector_medusa=np.array([1,1,1,1,1,0]), filter_vector_thanos=np.array([1,1,1,1,1,1]))
    bimanual_kuka.se2_arms_open_loop(qgoal_medusa, np.array([0,0,0]), medusa=True, se2_time = 20.0, force = 0.0, object_kg = 2.0, filter_vector_medusa=np.array([1,1,1,1,1,1]), filter_vector_thanos=np.array([1,1,1,1,1,0]))
...
if __name__ == '__main__':
    rospy.init_node("naive_open_loop_node")
    ...
    if not os.path.exists(f"data/naive/open_loop/circle/even"):
        os.makedirs(f"data/naive/open_loop/circle/even")
    np.save(f"data/naive/open_loop/circle/even/naive_angle_{angle}_path_{path_num}_MSE.npy", data)
