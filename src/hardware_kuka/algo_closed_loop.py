...
def algo_closed_loop(bimanual_kuka: BimanualKuka, goal_thanos, goal_medusa, angle = 30):
    ...
    horizon = 7
    obj2left, obj2right, vs = inhand_planner(current_thanos, current_medusa, goal_thanos, goal_medusa, dls_params, steps = horizon, angle = 60, palm_radius=0.03, kv = 20.0)
    ...
    bimanual_kuka.rotate_arms(90 * np.pi / 180, readjust_arms=False, rotate_time = 20.0)
    for desired_obj2left_se2, desired_obj2right_se2 in zip(desired_obj2left_se2s, desired_obj2right_se2s):
        bimanual_kuka.rotate_arms(-(90-angle) * np.pi / 180, grasp_force = 30.0, rotate_time = 20.0)
        bimanual_kuka.se2_arms(desired_obj2left_se2, medusa=False, se2_time = 15.0, force = 0.0, object_kg = 3.0, filter_vector_medusa=np.array([1,1,1,1,1,0]), filter_vector_thanos=np.array([1,1,1,1,1,1]))
        bimanual_kuka.rotate_arms((90-angle) * np.pi / 180, readjust_arms=False, rotate_time=20.0) #back to vertical
        bimanual_kuka.move_back(endtime=10.0)
        
        bimanual_kuka.rotate_arms((90-angle) * np.pi / 180, grasp_force = 30.0, rotate_time = 20.0)
        bimanual_kuka.se2_arms(desired_obj2right_se2, medusa=True, se2_time = 15.0, force = 0.0, object_kg = 3.0, filter_vector_medusa=np.array([1,1,1,1,1,1]), filter_vector_thanos=np.array([1,1,1,1,1,0]))
        bimanual_kuka.rotate_arms(-(90-angle) * np.pi / 180, readjust_arms=False, rotate_time = 20.0) #back to vertical
        bimanual_kuka.move_back(endtime=10.0)
